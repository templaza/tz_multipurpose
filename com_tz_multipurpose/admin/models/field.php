...
class TZ_MultipurposeModelField extends JModelAdmin
{
    ...
    public function getForm($data = array(), $loadData = true){
        $form = $this->loadForm('com_tz_multipurpose.field', 'field', array('control' => 'jform', 'load_data' => $loadData));
        if (empty($form)) {
            return false;
        }
        return $form;
...
//            $artOptFields   = $this -> _checkArticleFields($item -> id);
    ...
    public function getGroups($pk = null){
        ...
        $query -> join('LEFT',$db -> quoteName('#__tz_multipurpose_groupfield').' AS x ON x.groupid=g.id');
        $query -> join('LEFT',$db -> quoteName('#__tz_multipurpose_fields').' AS f ON x.fieldsid = f.id');
        $query -> where('f.id='.$pk);
        $db -> setQuery($query);
...
//            }
            ...
            if(count($groupid)>0){
                if($table -> id){
                    $query  = 'DELETE FROM #__tz_multipurpose_groupfield WHERE fieldsid ='.$table -> id;
                    $db -> setQuery($query);
...
//                }
                ...
                //Save into xref
                $value     = implode(',',$value);
                $query      = 'INSERT INTO #__tz_multipurpose_groupfield(`fieldsid`,`groupid`)'
                    .' VALUES'.$value;
                $db -> setQuery($query);
