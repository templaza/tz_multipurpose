...
//$saveOrderingUrl = 'index.php?option=com_tz_MULTIPURPOSE&task=fields.saveOrderAjax&tmpl=component';
//
...
window.addEvent('load', function() {
    var createBox = function(object,name,tz_count,imageurl){
        ...
        var tz_d = "image-select-" + tz_count,
            ...
            })).set('html', "<\i class=\"icon-file\"></i>&nbsp;<?php echo JText::_('COM_TZ_MULTIPURPOSE_BROWSE_SERVER');?>").inject(tz_a,'after'),
            tz_f = (new Element("a", {
                class: 'btn',
                "name": "tz_img_cancel_"+tz_count,
                html:'<i class="icon-refresh"></i>&nbsp;<?php echo JText::_('COM_TZ_MULTIPURPOSE_RESET');?>'
            })).inject(tz_b,'after'),
    ...
    var renderElement=function(tz_count_gb){
        ...
        var optionFields = function(){
            ...
            var myButton = new Element('a', {
                class: 'btn',
                html: '<i class="icon-plus"></i>&nbsp;<?php echo JText::_('COM_TZ_MULTIPURPOSE_ADD_NEW');?>',
                events: {
                    click: function(e){
                        e.stop();
                        ...
                        var myRemove = new Element('a',{
                            class: 'btn',
                            html:'<i class="icon-remove"></i>&nbsp;<?php echo JText::_('COM_TZ_MULTIPURPOSE_REMOVE');?>',
                            events:{
                                click: function(e){
                                    e.stop();
        ...
        switch (document.adminForm.type.value) {
            case 'textfield':
                ...
                var myDefault   = new Element('div',{
                    ...
                    html:'<i><?php echo JText::_('COM_TZ_MULTIPURPOSE_DEFAULT_VALUE');?></i>'
            ...
            case 'image':
                var myField = new Element('input', {
                    type: 'text',
                    'name': 'jform[option_value][]',
                    value: '<?php echo ($fields[0] -> type == 'image')?$fields[0] -> name:'';?>'
                });
                myField.inject($('fieldvalue'));
                var myDefault   = new Element('div',{
                    style:"font-size:11px; padding-top:5px;",
                ...
                createBox($('fieldvalue'),'jform[option_icon][]',0,'<?php echo ($fields[0] -> type == 'image')?$fields[0] -> image:'';?>');
                break;
            case 'link':
                var myDefault   = new Element('div',{
                    style:"font-size:11px; padding-top:5px;",
                    html:'<label></label><i><?php echo JText::_('COM_TZ_MULTIPURPOSE_DEFAULT_VALUE');?></i>'
                }).inject($('fieldvalue'));
                var linkDiv = new Element('div',{});
                linkDiv.inject($('fieldvalue'));
                var myLabel = new Element('label',{
                    html:'<strong><?php echo JText::_('COM_TZ_MULTIPURPOSE_LINK_TEXT');?></strong>',
                    styles:{
                        'font-size':'11px'
                    }
                });
                ...
                linkDiv.inject($('fieldvalue'));
                var myLabel = new Element('label',{
                    html:'<strong><?php echo JText::_('COM_TZ_MULTIPURPOSE_LINK_URL');?></strong>',
                    styles:{
                        'font-size':'11px'
                    }
                });
                ...
                linkDiv.inject($('fieldvalue'));
                var myLabel = new Element('label',{
                    html:'<strong><?php echo JText::_('COM_TZ_MULTIPURPOSE_OPEN_IN');?></strong>',
                    styles:{
                        'font-size':'11px'
                    }
                });
...
<!--                createBox($('fieldvalue'),'jform[option_icon][]',0,'--><?php //echo $fields[0] -> image;?><!--');-->
                ...
                break;
            default:
                $('fields').set('html', '<label><?php echo JText::_('COM_TZ_MULTIPURPOSE_OPTION_FIELD_VALUES_DESC');?></label>');
                break;
        }
    }
...
<form name="adminForm" method="post" id="field-form"
      action="index.php?option=com_tz_MULTIPURPOSE&view=field&layout=edit&id=<?php echo $this -> item -> id?>">
                    ...
                    <div class="control-group">
                        <div class="control-label">
                            <label width="100" for="title" class="hasTip"
                                   title="<?php echo JText::_('COM_TZ_MULTIPURPOSE_LABEL_TITLE')?>::<?php echo JText::_('COM_TZ_MULTIPURPOSE_LABEL_TITLE')?>">
                                <?php echo JText::_('COM_TZ_MULTIPURPOSE_LABEL_TITLE')?>
                    ...
                    <div class="control-group">
                        <div class="control-label">
                            <label width="100" for="groups" class="hasTip"
                                   title="<?php echo JText::_('COM_TZ_MULTIPURPOSE_FIELDS_GROUP_REQUIRED')?>::<?php echo JText::_('COM_TZ_MULTIPURPOSE_FIELDS_GROUP_REQUIRED_DESC')?>">
                                <?php echo JText::_('COM_TZ_MULTIPURPOSE_FIELDS_GROUP_REQUIRED')?>
                                <span class="star"> *</span>
                            </label>
                        </div>
                        <div class="controls">
                            <select multiple="multiple" size="10" id="groups"
                                ...
                                <option value="-1">
                                    <?php echo JText::_('COM_TZ_MULTIPURPOSE_OPTION_SELECT_GROUP');?>
                                </option>
                                <?php
                    ...
                    <div class="control-group">
                        <div class="control-label">
                            <label width="100" for="type" class="hasTip"
                                   title="<?php echo JText::_('COM_TZ_MULTIPURPOSE_TYPE')?>::<?php echo JText::_('COM_TZ_MULTIPURPOSE_TYPE_DESC')?>">
                                <?php echo JText::_('COM_TZ_MULTIPURPOSE_TYPE')?>
                                <span class="star"> *</span>
                            </label>
                        </div>
                        <div class="controls">
                            <select name="jform[type]" id="type" class="required" required="required">
                                <option value=""><?php echo JText::_('COM_TZ_MULTIPURPOSE_OPTION_SELECT_TYPE');?></option>
                                <option value="textfield"<?php echo ($this -> item-> type == 'textfield')?' selected="selected"':'';?>>
                                    <?php echo JText::_('COM_TZ_MULTIPURPOSE_TEXT_FIELD');?>
                                </option>
                                <option value="textarea"<?php echo ($this -> item-> type == 'textarea')?' selected="selected"':'';?>>
                                    <?php echo JText::_('COM_TZ_MULTIPURPOSE_TEXTAREA');?>
                                </option>
                                <option value="select"<?php echo ($this -> item-> type == 'select')?' selected="selected"':'';?>>
                                    <?php echo JText::_('COM_TZ_MULTIPURPOSE_DROP_DOWN_SELECTION');?>
                                </option>
                                <option value="multipleSelect"<?php echo (strtolower($this -> item-> type) == 'multipleselect')?' selected="selected"':'';?>>
                                    <?php echo JText::_('COM_TZ_MULTIPURPOSE_MULTI_SELECT_LIST');?>
                                </option>
                                <option value="radio"<?php echo ($this -> item-> type == 'radio')?' selected="selected"':'';?>>
                                    <?php echo JText::_('COM_TZ_MULTIPURPOSE_RADIO_BUTTONS');?>
                                </option>
                                <option value="checkbox"<?php echo ($this -> item-> type == 'checkbox')?' selected="selected"':'';?>>
                                    <?php echo JText::_('COM_TZ_MULTIPURPOSE_CHECK_BOX');?>
                                </option>
                                <option value="link"<?php echo ($this -> item-> type == 'link')?' selected="selected"':'';?>>
                                    <?php echo JText::_('COM_TZ_MULTIPURPOSE_LINK');?>
                                </option>
                                <option value="image"<?php echo ($this -> item-> type == 'image')?' selected="selected"':'';?>>
                                    <?php echo JText::_('COM_TZ_MULTIPURPOSE_IMG');?>
                                </option>
                            </select>
                        </div>
                    </div>
                    <div class="control-group">
                        <div class="control-label">
                            <label width="100" for="defaultvalue">
                                <?php echo JText::_('COM_TZ_MULTIPURPOSE_OPTION_FIELD_VALUES')?>:
                            </label>
                        </div>
                        <div class="controls" id="fields">
                            <label><?php echo JText::_('COM_TZ_MULTIPURPOSE_OPTION_FIELD_VALUES_DESC');?></label>
                        </div>
                    </div>
                    <div class="control-group">
                        <div class="control-label">
                    ...
                    <div class="control-group">
                        <div class="control-label">
                            <label width="100" for="jform_description">
                                <?php echo JText::_('COM_TZ_MULTIPURPOSE_DESCRIPTION');?>
                            </label>
                        </div>
                        <div class="controls">
